...
/** Starea meniului trăiește doar în aria-expanded; CSS o citește cu :has(). */
...
// Orice link din header (inclusiv logo-ul) închide meniul mobil.
...
const workshops = [...document.querySelectorAll('.catalog-card[data-workshop]')].map(
  (card) => ({
...
/** Data de azi în format ISO, calculată local (toISOString ar muta ziua în UTC). */
...
/** Ascunde atelierele care nu se potrivesc grupei de vârstă alese. */
const filterWorkshopsByAge = (ageSelect, workshopSelect) => {
  const selectedMinAge = Number(
    ageSelect.selectedOptions[0]?.dataset.minAge ?? 0
  );
...
const createChildCard = () => {
  ...
  workshopSelect.append(
    ...workshops.map((workshop) => new Option(workshop.title, workshop.title))
  );
...
/** Renumerotează cardurile și ascunde „Șterge” când a mai rămas un singur copil. */
...
nextWorkshopLink?.addEventListener('click', () => {
  ...
  const matchingAge = [...ageSelect.options].find(
    (option) => Number(option.dataset.minAge) >= workshop.minAge
  );
...
form.addEventListener('submit', async (event) => {
  ...
  const childLines = children.map(
    (child) => `Copil ${child.index}: ${child.name} — ${child.age} — ${child.workshop}`
  );
  ...
  try {
    ...
    console.error('Trimiterea formularului a eșuat:', error);
    setMessage(
...
document.querySelectorAll('.catalog-card, .step, footer').forEach((element) => {
...
new IntersectionObserver(
  ([entry]) => backToTop.classList.toggle('show', !entry.isIntersecting),
  { threshold: 0 }
